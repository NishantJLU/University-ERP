...
export default function CreateDepartmentDrawer({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.code || !formData.name) {
      toast.error("Please provide both department code and name.");
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({ code: "", name: "", description: "", building: "" });
        onClose();
  ...
  return (
    <Drawer
      ...
      description="Register a new academic department, campus building, and administrative hierarchy."
      footer={
        <>
          <button
            type="button"
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            type="text"
            required
            placeholder="e.g. AI, BIO, LAW, ARCH"
            value={formData.code}
          ...
          />
          <p className="text-[10px] text-slate-400 mt-1">Unique short identifier across the university</p>
        </div>
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-1">Campus Building / Block</label>
          <input
            type="text"
            placeholder="e.g. Block C, Science & Tech Wing"
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-1">Description / Academic Scope</label>
          <textarea
            rows={3}
            placeholder="Brief overview of research fields and academic offerings..."
